{{-- ===== resources/views/layouts/admin/partials/menu.blade.php ===== --}}
<ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
    ...
    @if (auth()->user()->role === 'admin')
        ...
        <x-menus.menu-bar :route="route('admin.bookings.index')" name="Quản lý Đặt vé" icon="fas fa-ticket-alt"
                          :route-group="['admin.bookings.*']"/>

        <x-menus.menu-bar :route="route('admin.companies.index')" name="Quản lý Nhà xe" icon="fas fa-building"
                          :route-group="['admin.companies.*']"/>


        <li class="nav-header">QUẢN LÝ DANH MỤC</li>

        <x-menus.menu-bar icon="fas fa-map-marked-alt" name="Quản lý Địa điểm"
        ...
        <x-menus.menu-bar :route="route('admin.bus-services.index')" name="Dịch vụ Xe" icon="fas fa-concierge-bell"
                          :route-group="['admin.bus-services.*']"/>

        <x-menus.menu-bar :route="route('admin.routes.index')" name="Quản lý Tuyến đường" icon="fas fa-route"
                          :route-group="['admin.routes.*']"/>

        <li class="nav-header">HỆ THỐNG</li>

        <x-menus.menu-bar icon="fas fa-cogs" name="Giao diện & Cài đặt"
    ...
    @elseif(auth()->user()->role === 'company')
        ...
        <x-menus.menu-bar :route="route('company.dashboard.index')" name="Tổng quan" icon="fas fa-tachometer-alt"
                          :route-group="['company.dashboard.*']"/>

        <x-menus.menu-bar :route="route('company.bookings.index')" name="Quản lý Đặt vé" icon="fas fa-ticket-alt"
                          :route-group="['company.bookings.*']"/>

        <x-menus.menu-bar icon="fas fa-bus" name="Quản lý Vận hành"
        ...
        <x-menus.menu-bar :route="route('company.profile.index')" name="Thông tin Nhà xe" icon="fas fa-id-card"
