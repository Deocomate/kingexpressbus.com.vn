            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-10 items-center">
                <div class="space-y-4 lg:col-span-2">
                    ...
                    <div class="flex flex-wrap gap-x-6 gap-y-2 text-base text-white/80">
                        <span class="inline-flex items-center gap-2"><i class="fa-solid fa-clock w-4 text-center"></i>
                            {{ \Carbon\Carbon::parse($trip->start_time)->format('H:i') }} -
                            {{ \Carbon\Carbon::parse($trip->end_time)->format('H:i') }}</span>
                        <span class="inline-flex items-center gap-2"><i
                                class="fa-solid fa-calendar-day w-4 text-center"></i>
                            {{ $bookingDate->format('d/m/Y') }}</span>
                        <span class="inline-flex items-center gap-2"><i class="fa-solid fa-couch w-4 text-center"></i>
                            {{ $trip->bus_name }}
                            ({{ $trip->bus_model ?? __('client.booking.common.updating') }})</span>
                    </div>
                </div>
        ...
        <div class="container mx-auto px-4 grid grid-cols-1 xl:grid-cols-3 gap-8">
            <form class="xl:col-span-2 space-y-8" method="POST" action="{{ route('client.booking.store') }}"
                ...
                <section class="bg-white border border-gray-100 rounded-3xl p-6 space-y-6">
                    <div class="flex items-center justify-between">
                        <h2 class="text-xl font-semibold text-gray-900">
                            {{ __('client.booking.create.trip_info_title') }}</h2>
                        <span
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <span class="text-sm text-gray-500">{!! trans_choice('client.booking.create.seats_left', $availableSeats, ['count' => $availableSeats]) !!}</span>
                            </div>
                        </div>
                    </div>
                </section>
                ...
                <section class="bg-white border border-gray-100 rounded-3xl p-6 space-y-6">
                    <div class="flex items-center justify-between">
                        <h2 class="text-xl font-semibold text-gray-900">
                            {{ __('client.booking.create.passenger_info_title') }}</h2>
                        <span
                        ...
                        <div>
                            ...
                            <select id="pickup_stop_id" name="pickup_stop_id" class="w-full" required>
                                ...
                                @if ($trip->available_hotel_pickup)
                                    <option value="hotel_pickup" @selected(old('pickup_stop_id', request('pickup_stop_id')) == 'hotel_pickup')>
                                        {{ __('client.booking.create.pickup_at_hotel') }}
                                    </option>
                                @endif
                                @foreach ($stops->where('stop_type', '!=', 'dropoff') as $point)
                                    <option value="{{ $point->id }}" data-address="{{ $point->address }}"
            ...
            <aside class="space-y-6">
                <div class="bg-white border border-gray-100 rounded-3xl p-6 space-y-4">
                    <h2 class="text-xl font-semibold text-gray-900">{{ __('client.booking.create.summary_title') }}
                    </h2>
                ...
                <div class="bg-blue-50 border border-blue-100 rounded-3xl p-6 space-y-4 text-sm text-blue-800">
                    <h3 class="text-base font-semibold text-blue-900">
                        {{ __('client.booking.create.amenities_title') }}</h3>
                ...
                <div class="bg-white border border-gray-100 rounded-3xl p-6 space-y-4">
                    <h3 class="text-xl font-semibold text-gray-900">{{ __('client.booking.create.support_title') }}
                    </h3>
                    ...
                    <div class="space-y-3">
                        @if (!empty($web_profile->hotline))
                            <a href="tel:{{ preg_replace('/[^\d+]/', '', $web_profile->hotline) }}"
                                ...
                                <span
                                    class="font-semibold text-gray-800">{{ __('client.booking.create.support_hotline') }}:
                                    {{ $web_profile->hotline }}</span>
                            </a>
                        @endif
                        @if (!empty($web_profile->phone))
                            <a href="tel:{{ preg_replace('/[^\d+]/', '', $web_profile->phone) }}"
                                ...
                                <span
                                    class="font-semibold text-gray-800">{{ __('client.booking.create.support_call_center') }}:
                                    {{ $web_profile->phone }}</span>
                            </a>
                        @endif
                        @if (!empty($web_profile->email))
                            <a href="mailto:{{ $web_profile->email }}"
                                ...
                                <span
                                    class="font-semibold text-gray-800">{{ __('client.booking.create.support_email') }}:
                                    {{ $web_profile->email }}</span>
                            </a>
                        @endif
        ...
        <script>
            $(document).ready(function() {
                ...
                function formatStop(stop) {
                    ...
                    var $stop = $(
                        '<span>' + stop.text + '</span><br/><small class="text-gray-500">' + (stop.element
                            .getAttribute('data-address') || '') + '</small>'
                    );
                    return $stop;
                }
